...
/*func (j *Jar) save(now time.Time) error {
	...
	if err := f.Truncate(0); err != nil {
		return errors.New(err.Error() + "cannot truncate file")
	}
...
func (j *Jar) Unmarshal(r []byte) error {
	buffer := bytes.NewBuffer(r)
	return j.MergeFrom(buffer)
}
...
func (j *Jar) ToJSON() ([]byte, error) {
	return json.Marshal(j.entries)
}
func (j *Jar) FromJSON(data []byte) error {
	return json.Unmarshal(data, &j.entries)
	//return json.Marshal(j.entries)
}
func NewJarFromJSON(data []byte) *Jar {
	jar, _ := New(nil)
	jar.FromJSON(data)
	jar.deleteExpired(time.Now())
	return jar
}
